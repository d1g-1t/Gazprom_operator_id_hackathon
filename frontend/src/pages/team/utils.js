...
export function CreateNodesAndEdges() {
  ...
  const currentUsers = currentTeam?.usersId.map((item) =>
    companyStructure.find((i) => i.id === item)
  );
  const teamLead = currentUsers?.find((i) => i.grade === "Team Lead");
  const restTeamUsers = currentUsers?.filter((i) => i.grade !== "Team Lead");
